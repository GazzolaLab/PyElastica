...
head_element = int(6)
# arm_1_element = int((n_elem - head_element) / 2)
# arm_2_element = int((n_elem - head_element) / 2)
# head_element_start_idx = int((n_elem - head_element) / 2)
# head_element_end_idx = head_element_start_idx + head_element
#
#
radius_tip = 0.025
radius_base = 0.03
radius_head = 0.05
#
# radius_along_rod = np.linspace(radius_tip, radius_base, arm_1_element)
# for i in range(head_element):
#     radius_along_rod = np.hstack((radius_along_rod, radius_head))
# radius_along_rod = np.hstack((radius_along_rod,np.linspace(radius_tip, radius_base, arm_2_element)[::-1]))
...
def make_tappered_arm(
    ...
    """
    This function is used to reset the rod properties for a varying radius. This function computes,
    new mass, mass moment of inertia, shear, bend matrices and volume of rod elements.
    :param rod:
...
def make_two_arm(
    rod,
    beta,
    # head_element_start_idx,
    # head_element_end_idx,
    direction,
    normal,
    start,
    head_n_elems,
    ...
    Angle between arms and head is determined by the user input beta. This function
    computes directors and positions for three segments, which are first arm, head and
    second arm. Finally, rest curvature and strains are computed for the final configuration.
    :param rod:
    :param beta:
    :param head_element_start_idx:
    :param head_element_end_idx:
    :param direction:
    :param normal:
    :param start:
    :param head_n_elems:
    ...
    # Compute the number of elements in arm
    ...
    # Construct the directors for the first arm of octopus
    # d3_segment1 = np.array([0.0, np.sin(alpha), np.cos(alpha)])
    # d3_segment1 /= np.linalg.norm(d3_segment1)

    d3_segment1 = np.cos(alpha) * direction + np.sin(alpha) * normal
    d3_segment1 /= np.linalg.norm(d3_segment1)

    # d2_segment1 = np.array([0.0, np.cos(alpha), -np.sin(alpha)])
    # d2_segment1 /= np.linalg.norm(d2_segment1)
    # d1_segment1 = np.cross(d3_segment1, d2_segment1)
    # directors_segment1 = np.vstack((d1_segment1, d2_segment1, d3_segment1)).reshape(
    ...
    # d2_segment2 = normal / np.linalg.norm(normal)
    # d1_segment2 = np.cross(d3_segment2, d2_segment2)
    # directors_segment2 = np.vstack((d1_segment2, d2_segment2, d3_segment2)).reshape(
    #     MaxDimension.value(), MaxDimension.value(), 1
    # )

    # Constructor the directors for the second arm of octopus
    # Set the second arm of octopus mirror symmetric to first arm
    # d3_segment3 = np.array([0.0, np.sin(-alpha), np.cos(-alpha)])
    # d3_segment3 /= np.linalg.norm(d3_segment3)
    d3_segment3 = np.cos(-alpha) * direction + np.sin(-alpha) * normal
    d3_segment3 /= np.linalg.norm(d3_segment3)

    # d2_segment3 = np.array([0.0, np.cos(-alpha), -np.sin(-alpha)])
    # d2_segment3 /= np.linalg.norm(d2_segment3)
    # d1_segment3 = np.cross(d3_segment3, d2_segment3)
    # directors_segment3 = np.vstack((d1_segment3, d2_segment3, d3_segment3)).reshape(
    #     MaxDimension.value(), MaxDimension.value(), 1
    # )
    #
    # shearable_rod.director_collection[..., :head_element_start_idx] = directors_segment1
    # shearable_rod.director_collection[
    #     ..., head_element_start_idx : head_element_end_idx
    # ] = directors_segment2
    # shearable_rod.director_collection[..., head_element_end_idx :] = directors_segment3
...
radius_along_rod, position = make_two_arm(
    shearable_rod,
    240,
    direction,
    normal,
...
plt.plot(
    shearable_rod.position_collection[2], shearable_rod.position_collection[1], "o-"
)
plt.show()
...
# Third pack of hierarchical muscles
thrid_muscle_segment = SpatiallyInvariantSplineHierarchy(
    Union(Gaussian(0.20), Gaussian(0.08), Gaussian(0.05))
)
# apply filters to change magnitude
thrid_muscle_segment.apply_filter(0, ScalingFilter, 32)
thrid_muscle_segment.apply_filter(1, ScalingFilter, 8)
thrid_muscle_segment.apply_filter(2, ScalingFilter, 2)

fourth_muscle_segment = SpatiallyInvariantSplineHierarchy(
    Union(Gaussian(0.10), Gaussian(0.05), Gaussian(0.03))
)
# apply filters to change magnitude
fourth_muscle_segment.apply_filter(0, ScalingFilter, 20)
fourth_muscle_segment.apply_filter(1, ScalingFilter, 10)
fourth_muscle_segment.apply_filter(2, ScalingFilter, 3)


first_muscle_mapper = SpatiallyInvariantSplineHierarchyMapper(
    first_muscle_segment, (0.25, 0.40)
)
second_muscle_mapper = SpatiallyInvariantSplineHierarchyMapper(
    second_muscle_segment, (0.05, 0.25)
)
...
segments_of_muscle_hierarchies = SplineHierarchySegments(
    second_muscle_mapper, first_muscle_mapper, third_muscle_mapper, fourth_muscle_mapper
)
...
def four_segment_activation(time_v):
    activation_arr = np.zeros((13 * 4,))
    ...
    activation_arr[14:17] = ramped_up(time_v - 0.3, 0.1)
    # bottom boys
    activation_arr[17:26] = ramped_up(time_v, 0.1)
    ...
    # The third muscle segment that controls the shoulder
    # top_level
    activation_arr[13 + 26] = ramped_up(time_v - 0.6, 0.1)
    # mid_levels
    activation_arr[14 + 26 : 17 + 26] = ramped_up(time_v - 0.3, 0.1)
    # bottom boys
    activation_arr[17 + 26 :] = ramped_up(time_v, 0.1)

    # The seconc muscle segment that controls the finger
    activation_arr[0 + 26] = ramped_up(time_v - 1.0, 0.1)
    # mid_levels
    activation_arr[1 + 26 : 4 + 26] = ramped_up(time_v - 0.9, 0.1)
    # bottom boys
    activation_arr[4 + 26 : 13 + 26] = ramped_up(time_v - 0.8, 0.1)
...
filename_video = "two_arm_simulation.mp4"
plot_video(
    pp_list, video_name=filename_video, margin=0.4, fps=20, step=10,
)

filename_activation_muscle_torque_video = "two_arm_activation_muscle_torque.mp4"
plot_video_actiavation_muscle(
