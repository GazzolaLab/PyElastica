...
base_length = 100 * length_scale
n_elem = 350

number_of_rotations = 0.487125 * base_length / length_scale
time_start_twist = 10
time_twist = 1 * number_of_rotations
time_relax = 0
final_time = time_relax + time_twist + time_start_twist

time_scale = 1
dt = (0.25 * base_length / n_elem) * time_scale
...
base_radius = 0.74 * length_scale
base_area = np.pi * base_radius ** 2
I = np.pi / 4 * base_radius ** 4
volume = base_area * base_length
density = 1090 * mass_scale / (length_scale) ** 3
mass = density * volume
nu = 3e-3
...
force_mag = 250
F_pulling_scalar = force_mag * (np.pi ** 2) * E * I / (base_length ** 2)
...
manufacture_sim.append(sherable_rod)

# Add damping
# manufacture_sim.dampen(sherable_rod).using(
...
class manufacturingBC(ConstraintBase):
    """ """
    ...
    def constrain_rates(self, rod, time):
        if time > self.twisting_time + self.time_twis_start:
            ...
            rod.velocity_collection[2, int(rod.n_elems / 2)] -= 1e-7 * length_scale


manufacture_sim.constrain(sherable_rod).using(
    manufacturingBC,
...
manufacture_sim.add_forcing_to(sherable_rod).using(
    EndpointForces,
    np.zeros(
        3,
    ),
...
if self_contact:
    manufacture_sim.connect(sherable_rod, sherable_rod).using(
        SelfContact, k=shear_modulus, nu=3e-3
    )
...
manufacture_sim.collect_diagnostics(sherable_rod).using(
    RodCallBack,
    step_skip=step_skip,
...
# finalize simulation
manufacture_sim.finalize()

# Run the simulation
time_stepper = PositionVerlet()
integrate(time_stepper, manufacture_sim, final_time, total_steps)

# plotting the videos
filename_video = "monocoil_manufacturing.mp4"
plot_video_with_surface(
    ...
    vis3D=True,
    vis2D=True,
    x_limits=[-40 * base_radius, 40 * base_radius],
    y_limits=[-0.1 * base_length, 1.3 * base_length],
    z_limits=[-40 * base_radius, 40 * base_radius],
)
