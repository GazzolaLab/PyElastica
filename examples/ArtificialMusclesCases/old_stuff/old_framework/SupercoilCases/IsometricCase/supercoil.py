...
kappa_change = 0.0557691  # 0 is no actuation
...
for fiber in range(total_number_of_helical_rods):
    ...
    if Isometric:
        constrain_start_positions = np.zeros_like(
            muscle_rods[fiber].position_collection
        )
        constrain_start_directors = np.zeros_like(
            muscle_rods[fiber].director_collection
        )
        muscle_sim.constrain(muscle_rods[fiber]).using(
            IsometricBC,
...
internal_force = 0
for i in range(total_number_of_helical_rods):
    internal_force += np.array(post_processing_dict_list[i]["internal_force"])


print(
    "internal_force_difference:" + str((internal_force[-1, 2, 0] * 1e6) - 2.550001556)
)
