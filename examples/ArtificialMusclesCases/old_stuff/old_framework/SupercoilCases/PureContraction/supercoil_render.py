...
# Setup (USER DEFINE)
DATA_PATH = "data/supercoiled_muscle.dat"  # Path to the simulation data
SAVE_PICKLE = True

# Rendering Configuration (USER DEFINE)
OUTPUT_FILENAME = "pov_supercoil"
...
if __name__ == "__main__":
    ...
    for i in range(len(data)):
        xs[i] = (
            np.array(data[i]["position"]) * 1000
        )  # shape: (timelength, 3, num_element)
        base_radius[i] = (
            np.array(data[i]["radius"])[:, 0:1] * np.ones_like(xs[i][:, 0, :]) * 1000
