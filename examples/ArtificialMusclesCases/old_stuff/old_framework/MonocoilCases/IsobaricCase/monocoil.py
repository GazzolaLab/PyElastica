...
from examples.ArtificialMusclesCases.TestingBC import IsometricBC
...
end_temperature = 120
thermal_expansion_coeficient = 5.47895e-3  # 5.4865e-3 #5.4025e-3 #
...
if Thompson_model:
    muscle_sim._memory_blocks[0].compute_internal_forces_and_torques(0)
    force_thompson[:] = muscle_rod.internal_forces[:]
    torque_thompson[:] = muscle_rod.internal_torques[:]
else:
...
print(
    (
        (positions[-1, 2, -1] - positions[-1, 2, 0])
        - (positions[0, 2, -1] - positions[0, 2, 0])
    )
    / (positions[0, 2, -1] - positions[0, 2, 0])
)
