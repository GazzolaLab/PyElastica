...
def _get_zero_array(dim: int, ndim: int):
    if ndim == 1:
        return 0.0
    if ndim == 2:
...
@njit(cache=True)
def _trapezoidal(array_collection: NDArray[np.floating]):
    """
...
@njit(cache=True)
def _two_point_difference(array_collection: NDArray[np.floating]):
    """
...
@njit(cache=True)
def _difference(vector: NDArray[np.floating]):
    """
...
@njit(cache=True)
def _average(vector: NDArray[np.floating]):
    """
...
@njit(cache=True)
def _clip_array(input_array: NDArray[np.floating], vmin: float, vmax: float):
    """
...
@njit(cache=True)
def _isnan_check(array: NDArray[Any]):
    """
