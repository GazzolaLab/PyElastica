...
def construct_memory_block_structures(systems):
    ...
    temp_list_for_magnetic_rod_systems = []

    for system_idx, sys_to_be_added in enumerate(systems):

        if issubclass(sys_to_be_added.__class__, MagneticRod):
            temp_list_for_magnetic_rod_systems.append(sys_to_be_added)
    ...
    if temp_list_for_magnetic_rod_systems:
        _memory_blocks.append(
            MemoryBlockMagneticRod(temp_list_for_magnetic_rod_systems)
        )
