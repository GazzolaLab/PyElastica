...
def format_vector_shape(vector_collection: NDArray[np.floating]):
    """
...
def format_matrix_shape(matrix_collection: NDArray[np.floating]):
    """
    ...
    def assert_proper_square(num1: int):
        sqrt_num = isqrt(num1)
...
def skew_symmetrize(vector: NDArray[np.floating]):
    vector = format_vector_shape(vector)
    return _skew_symmetrize(vector)


def inv_skew_symmetrize(matrix_collection: NDArray[np.floating]):
    """
...
def rotate(matrix: NDArray[np.floating], scale: float, axis: NDArray[np.floating]):
    """
