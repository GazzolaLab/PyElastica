...
class GravityForces(NoForces):
    ...
    def __init__(
        self, acc_gravity: NDArray[np.floating] = np.array([0.0, -9.80665, 0.0])
    ) -> None:
        """
...
class EndpointForces(NoForces):
    ...
    def compute_end_point_forces(
        ...
        """
        factor: np.floating = min(1.0, time / ramp_up_time)
...
class UniformTorques(NoForces):
    ...
    def __init__(
        self,
        torque: np.floating,
        direction: NDArray[np.floating] = np.array([0.0, 0.0, 0.0]),
    ) -> None:
        """
...
class UniformForces(NoForces):
    ...
    def apply_forces(self, rod: SystemType, time: np.floating = 0.0) -> None:
...
class MuscleTorques(NoForces):
    ...
    def apply_torques(self, rod: SystemType, time: np.floating = 0.0) -> None:
