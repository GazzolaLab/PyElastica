...
def levi_civita_tensor(dim: int):
    """
...
@njit(cache=True)
def _batch_dot(first_vector: NDArray[np.floating], second_vector: NDArray[np.floating]):
    """
...
@njit(cache=True)
def _batch_norm(vector: NDArray[np.floating]):
    """
...
@njit(cache=True)
def _batch_vector_sum(vector1: NDArray[np.floating], vector2: NDArray[np.floating]):
    """
...
@njit(cache=True)
def _batch_matrix_transpose(input_matrix: NDArray[np.floating]):
    """
