...
def test_at_returns_block_view():
    ...
    assert block.shape[1] == 15

    for sys_index in range(block.n_systems):
        block_view = block.at(sys_index)
        # assert block_view.shape[0] == expected_depth  # depth may change with dummy variables
        assert block_view.shape[1] == n_elems_per_system[sys_index] + 1
