...
void compute_geometry_from_state(BlockType& block) {
    // Get variable views
    ...
    const Eigen::Index n_elems = lengths.cols();
    #ifdef ELASTICAPP_USE_THREADING
...
template<typename BlockType>
void compute_all_dilatations(BlockType& block) {
    // First compute geometry from state (lengths, tangents, radius)
    ...
    auto voronoi_lengths = elastica::average_kernel(lengths);

    // Compute voronoi_dilatation = voronoi_lengths / rest_voronoi_lengths
    const Eigen::Index n_voronoi = voronoi_dilatation.cols();
    #ifdef ELASTICAPP_USE_THREADING
    #ifdef ELASTICAPP_NUM_THREADS
    #pragma omp parallel for num_threads(ELASTICAPP_NUM_THREADS)
    #else
    ...
    #endif
    #endif
    for (Eigen::Index k = 0; k < n_voronoi; ++k) {
...
template<typename BlockType>
void compute_shear_stretch_strains(BlockType& block) {
    // First compute all dilatations (which also computes geometry from state)
    ...
    // z_vector = [0, 0, 1] (broadcast to all elements)
    ...
    // sigma is (3, n_elems)
    const Eigen::Index n_elems = sigma.cols();
    ...
    for (Eigen::Index k = 0; k < n_elems; ++k) {
        ...
        // director[:, k] is [d00, d10, d20, d01, d11, d21, d02, d12, d22]
        ...
        sigma(0, k) = dil * result0 - z_vector[0];
        sigma(1, k) = dil * result1 - z_vector[1];
        sigma(2, k) = dil * result2 - z_vector[2];
    }
}
...
template<typename BlockType>
void compute_internal_shear_stretch_stresses_from_model(BlockType& block) {
    // First compute shear/stretch strains (which also computes all dilatations)
...
template<typename BlockType>
void compute_internal_forces(BlockType& block) {
    // First compute internal shear/stretch stresses (which also computes strains and dilatations)
    ...
    const Eigen::Index n_elems = director.cols();

    // Temporary matrix for cosserat_internal_stress
    using namespace elastica;
    ElasticaMatrix cosserat_internal_stress(3, n_elems);
    ...
    #ifdef ELASTICAPP_USE_THREADING
    #ifdef ELASTICAPP_NUM_THREADS
    #pragma omp parallel for num_threads(ELASTICAPP_NUM_THREADS) collapse(2)
    #else
    ...
    for (Eigen::Index i = 0; i < 3; ++i) {
        for (Eigen::Index k = 0; k < n_elems; ++k) {
            ...
            for (Eigen::Index j = 0; j < 3; ++j) {
                ...
                Eigen::Index director_idx = j * 3 + i;
                sum += director(director_idx, k) * internal_stress(j, k);
            }
            cosserat_internal_stress(i, k) = sum;
        }
    }

    // Divide by dilatation
    #ifdef ELASTICAPP_USE_THREADING
    #ifdef ELASTICAPP_NUM_THREADS
    #pragma omp parallel for num_threads(ELASTICAPP_NUM_THREADS) collapse(2)
    #else
    ...
    for (Eigen::Index i = 0; i < 3; ++i) {
        for (Eigen::Index k = 0; k < n_elems; ++k) {
            cosserat_internal_stress(i, k) /= dilatation(0, k);
        }
    }
...
// Updates: kappa
template<typename BlockType>
void compute_bending_twist_strains(BlockType& block) {
    // Get variable views
    auto director = block.template get<system::cosserat_rod::Director>();
    auto rest_voronoi_lengths = block.template get<system::cosserat_rod::RestVoronoiLengths>();
    ...
    const Eigen::Index n_elems = director.cols();
    const Eigen::Index n_voronoi = kappa.cols();
    ...
    using namespace elastica;
    ElasticaMatrix temp(3, n_voronoi);
    ...
    for (Eigen::Index k = 0; k < n_voronoi; ++k) {
        ...
        // director[:, k] is [d00, d10, d20, d01, d11, d21, d02, d12, d22]
        // Row 0: [d00, d01, d02] = [director(0,k), director(3,k), director(6,k)]
        // Row 1: [d10, d11, d12] = [director(1,k), director(4,k), director(7,k)]
        // Row 2: [d20, d21, d22] = [director(2,k), director(5,k), director(8,k)]

        // For director[k]:
        double d00_k = director(0, k);
        double d10_k = director(1, k);
        double d20_k = director(2, k);
        double d01_k = director(3, k);
        double d11_k = director(4, k);
        double d21_k = director(5, k);
        double d02_k = director(6, k);
        double d12_k = director(7, k);
        double d22_k = director(8, k);

        // For director[k+1]:
        double d00_k1 = director(0, k + 1);
        double d10_k1 = director(1, k + 1);
        double d20_k1 = director(2, k + 1);
        double d01_k1 = director(3, k + 1);
        double d11_k1 = director(4, k + 1);
        double d21_k1 = director(5, k + 1);
        double d02_k1 = director(6, k + 1);
        double d12_k1 = director(7, k + 1);
        double d22_k1 = director(8, k + 1);

        // Compute inv_rotate: cross product between consecutive director frames
        // Python: vector_collection[0, k] = (d2[k+1] · d1[k]) - (d1[k+1] · d2[k])
        temp(0, k) = (d20_k1 * d10_k + d21_k1 * d11_k + d22_k1 * d12_k) -
                     (d10_k1 * d20_k + d11_k1 * d21_k + d12_k1 * d22_k);

        // Python: vector_collection[1, k] = (d0[k+1] · d2[k]) - (d2[k+1] · d0[k])
        temp(1, k) = (d00_k1 * d20_k + d01_k1 * d21_k + d02_k1 * d22_k) -
                     (d20_k1 * d00_k + d21_k1 * d01_k + d22_k1 * d02_k);

        // Python: vector_collection[2, k] = (d1[k+1] · d0[k]) - (d0[k+1] · d1[k])
        temp(2, k) = (d10_k1 * d00_k + d11_k1 * d01_k + d12_k1 * d02_k) -
                     (d00_k1 * d10_k + d01_k1 * d11_k + d02_k1 * d12_k);
    }

    // Compute kappa = temp / rest_voronoi_lengths
    // Python: kappa[i, k] = temp[i, k] / rest_voronoi_lengths[k]
    #ifdef ELASTICAPP_USE_THREADING
    #ifdef ELASTICAPP_NUM_THREADS
    #pragma omp parallel for num_threads(ELASTICAPP_NUM_THREADS) collapse(2)
    #else
    ...
    for (Eigen::Index i = 0; i < 3; ++i) {
        for (Eigen::Index k = 0; k < n_voronoi; ++k) {
            kappa(i, k) = temp(i, k) / rest_voronoi_lengths(0, k);
        }
    }
}
...
template<typename BlockType>
void compute_internal_bending_twist_stresses_from_model(BlockType& block) {
    // First compute bending/twist strains
    ...
    for (Eigen::Index k = 0; k < n_voronoi; ++k) {
        ...
        // Compute bend_matrix @ temp for voronoi k
        // result = bend_matrix * temp[:, k]
        internal_couple(0, k) = m00 * temp(0, k) + m01 * temp(1, k) + m02 * temp(2, k);
        internal_couple(1, k) = m10 * temp(0, k) + m11 * temp(1, k) + m12 * temp(2, k);
        internal_couple(2, k) = m20 * temp(0, k) + m21 * temp(1, k) + m22 * temp(2, k);
    }
}
...
template<typename BlockType>
void compute_dilatation_rate(BlockType& block) {
    // Get variable views
    auto position = block.template get<system::cosserat_rod::Position>();
    ...
    ElasticaMatrix r_plus_one_dot_v(1, n_elems);
    #ifdef ELASTICAPP_USE_THREADING
    #ifdef ELASTICAPP_NUM_THREADS
    #pragma omp parallel for num_threads(ELASTICAPP_NUM_THREADS)
    #else
    ...
    // Dot product of position[:-1] and velocity[1:] (both have n_elems elements)
    ElasticaMatrix r_dot_v_plus_one(1, n_elems);
    #ifdef ELASTICAPP_USE_THREADING
    #ifdef ELASTICAPP_NUM_THREADS
    #pragma omp parallel for num_threads(ELASTICAPP_NUM_THREADS)
    #else
...
// This is a template function that will be implemented later
template<typename BlockType>
void compute_internal_torques(BlockType& block) {
    // First compute internal bending/twist stresses and dilatation rate
    ...
    const Eigen::Index n_voronoi = voronoi_dilatation.cols();
    const Eigen::Index n_elems = internal_torques.cols();
    const Eigen::Index n_nodes = n_elems + 1;

    using namespace elastica;

    // Compute voronoi_dilatation_inv_cube_cached = 1.0 / voronoi_dilatation^3
    ElasticaMatrix voronoi_dilatation_inv_cube_cached(1, n_voronoi);
    #ifdef ELASTICAPP_USE_THREADING
    #ifdef ELASTICAPP_NUM_THREADS
    #pragma omp parallel for num_threads(ELASTICAPP_NUM_THREADS)
    #else
    ...
    ElasticaMatrix internal_couple_scaled(3, n_voronoi);
    #ifdef ELASTICAPP_USE_THREADING
    #ifdef ELASTICAPP_NUM_THREADS
    #pragma omp parallel for num_threads(ELASTICAPP_NUM_THREADS) collapse(2)
    #else
    ...
    ElasticaMatrix bend_twist_couple_2D(3, n_nodes);
    ...
    ElasticaMatrix kappa_cross_internal_couple(3, n_voronoi);
    ...
    ElasticaMatrix bend_twist_couple_3D_input(3, n_voronoi);
    #ifdef ELASTICAPP_USE_THREADING
    #ifdef ELASTICAPP_NUM_THREADS
    #pragma omp parallel for num_threads(ELASTICAPP_NUM_THREADS) collapse(2)
    #else
    ...
    ElasticaMatrix bend_twist_couple_3D(3, n_nodes);
    ...
    ElasticaMatrix director_tangents(3, n_elems);
    #ifdef ELASTICAPP_USE_THREADING
    #ifdef ELASTICAPP_NUM_THREADS
    #pragma omp parallel for num_threads(ELASTICAPP_NUM_THREADS) collapse(2)
    #else
    ...
    ElasticaMatrix shear_stretch_couple(3, n_elems);
    ...
    ElasticaMatrix J_omega_upon_e(3, n_elems);
    #ifdef ELASTICAPP_USE_THREADING
    #ifdef ELASTICAPP_NUM_THREADS
    #pragma omp parallel for num_threads(ELASTICAPP_NUM_THREADS)
    #else
    ...
    ElasticaMatrix lagrangian_transport(3, n_elems);
    ...
    ElasticaMatrix unsteady_dilatation(3, n_elems);
    #ifdef ELASTICAPP_USE_THREADING
    #ifdef ELASTICAPP_NUM_THREADS
    #pragma omp parallel for num_threads(ELASTICAPP_NUM_THREADS) collapse(2)
    #else
