...
namespace elastica {
  ...
  namespace states {
    ...
    //**************************************************************************
    /*!\brief Expression template object for multiplication between SE3 states
     * and TimeDelta
     * \ingroup states
     *
     * SE3TimeDeltaMultExpr class represents the compile time expression for
     * multiplication between a SE3 state and a time delta type
     *
     * \tparam ST Type of the state
     * \tparam TDT Type of the time-delta
     */
    template <typename ST, typename TDT>
    class SE3TimeDeltaMultExpr : public StateTimeDeltaMultExprBase<
        SE3Base<SE3TimeDeltaMultExpr<ST, TDT>>> {
     public:
      //**Type definitions******************************************************
      //! Type of this SE3TimeDeltaMultExpr instance.
      using This = SE3TimeDeltaMultExpr<ST, TDT>;
      //! Base type of this SE3TimeDeltaMultExpr instance.
      using BaseType = StateTimeDeltaMultExprBase<SE3Base<This>>;
      ...
      //**Constructor***********************************************************
      /*!\brief Constructor for the SE3TimeDeltaMultExpr class.
       *
       * \param state The SE3 state of the multiplication expression.
       * \param time_delta The time-delta of the multiplication expression.
       */
      inline SE3TimeDeltaMultExpr(const ST& state,
      ...
      */
      ...
      //**Left operand access***************************************************
      /*!\brief Returns the left-hand side SE3 state operand.
      //
      // \return The left-hand side SE3 state operand.
      */
    ...
    //**************************************************************************
    /*!\brief Operator for the multiplication of a SE3 and TimeDelta
     * \ingroup states
     *
     * \details
     * This operator represents the multiplication between a SE3 and a
     * TimeDelta value:
     ...
     * SE3<std::vector<double>, tags::DerivativeTag> a;
     * TimeDelta dt{2.0};
     *
     ...
     * between the argument SE3 and time-delta.
     *
    ...
    inline decltype(auto) operator*(SE3Base<ST> const& state,
                                    TimeDelta const time_delta) noexcept {
      using ReturnType = const SE3TimeDeltaMultExpr<ST, TimeDelta>;
    ...
    inline decltype(auto) operator*(TimeDelta const time_delta,
                                    SE3Base<ST> const& state) noexcept {
      using ReturnType = const SE3TimeDeltaMultExpr<ST, TimeDelta>;
      return ReturnType(*state, time_delta);
    }
    //**************************************************************************
