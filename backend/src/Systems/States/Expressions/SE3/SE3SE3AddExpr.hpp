...
namespace elastica {
  ...
  namespace states {
     ...
     * \ingroup states
     *
     * SE3SE3AddExpr class represents the compile time expression for
    ...
    class SE3SE3AddExpr
        : public StateAddExpr<SE3Base<SE3SE3AddExpr<STL, STR>>> {
     public:
      //**Type definitions******************************************************
      //! This type
      using This = SE3SE3AddExpr<STL, STR>;
      //! Base type of this instance.
      using BaseType = StateAddExpr<SE3Base<This>>;
      ...
      //**Constructor***********************************************************
      /*!\brief Constructor for the SE3SE3AddExpr class.
       *
       ...
       */
      inline SE3SE3AddExpr(const STL& lhs, const STR& rhs) noexcept
          : lhs_(lhs), rhs_(rhs) {
      ...
      /*!\brief Returns the right-hand side SE3 state operand.
       *
    ...
    //**************************************************************************
    /*!\brief Operator for the addition of two SE3 states
     * \ingroup states
     *
     * \details
     * This operator represents the addition of two SE3 states:
     ...
     * SE3<std::vector<double>, tags::DerivativeTag> a, b;
    ...
    inline decltype(auto) operator+(SE3Base<STL> const& lhs,
                                    SE3Base<STR> const& rhs) {
      if ((*lhs).size() != (*rhs).size()) {
        throw std::invalid_argument("State sizes do not match");
      }
      using ReturnType = const SE3SE3AddExpr<STL, STR>;
