...
class rod(object):
    ...
    # k0 (k_intrinsic)
    def __init__(
            self, n, r, rho, yng_mod, shr_mod, gamma=0.0, *args, **kwargs):
        ...
        self.d1 = np.array([[0., 0., 1.] for i in range(self.n)])
        ...
        self.radii = np.ones(n) * np.sqrt(1./np.pi)
        ...
        self.I_org = np.array([
            self.A_org[i]**2 / (4. * np.pi) * np.array([1, 1, 2])
            for i in range(self.n)])

        self.J_org = self.rho * (self.I_org * self.l_org[:, None])
        self.B_org = self.I_org * np.array(
            [self.yng_mod, self.yng_mod, self.shr_mod])
        self.S_org = self.A_org[:, None] * np.array(
                                        [self.shr_mod * self.alpha_c,
                                         self.shr_mod * self.alpha_c,
                                         self.yng_mod])
    ...
    def compute_e(self,):
        return np.linalg.norm(self.r[1:] - self.r[0:-1], axis=1) \
            / self.l_org
    ...
    def compute_Q(self,):
        return np.array(
            [self.d1[:], self.d2[:], self.d3[:]]).transpose(1, 0, 2)
    ...
    def compute_B_vorn(self,):
        ...
        return (Biplus1 + Bi) / (2. * self.D_org[:, None])
    ...
    def inverse_rotate_v(self,):
        ...
        theta = np.around((np.trace(R, axis1=1, axis2=2) - 1.) / 2., 12)
        ...
        if np.any(far):
            theta_u[far] = (theta[far, None, None]
                            / (2. * np.sin(theta[far, None, None]))) \
    ...
    def inverse_rotate(self,):
        ...
        for i in range(self.n-1):
            ...
            if np.isclose(theta, 0.0, atol=TOL):
                ...
                theta_u = (R - R.T) * theta / (2. * np.sin(theta))
                k[i] = np.array(
                    [theta_u[2, 1], theta_u[0, -1], theta_u[1, 0]]) \
                    / self.D_org[i]
        return k
    ...
    def rotate(self, theta, u):
        ...
        for i in range(self.n):
            ...
            rot_matrix[i] = tmp + U_mat @ (
                s_angle * tmp + (1. - c_angle) * U_mat)
    ...
    def compute_rhs_linearmom(self,):
        ...
        #             for i in range(self.n)])
        tmp = ((self.Q.transpose(0, 2, 1) * (
            self.S_org / self.e[:, None])[:, None])
            @ self.shearStrain[:, :, None]).transpose(0, 2, 1)[:, 0, :]
        fv = -self.gamma * self.v
        grav = self.gravity * (-9.81 * np.array([0., 0., 1.]))
        dvdt = (self.delta_h_op(tmp) + self.f_ext + fv) / self.mass[:, None] \
            + grav
    ...
    def compute_rhs_angularmom(self, dt_prev):
        ...
        shearstretch = np.array([
                    -np.cross(np.dot(self.Q[i], self.tangent[i]),
                                np.dot(np.diag(
                                    self.S_org[i]), self.shearStrain[i]))
                    * self.l_org[i] for i in range(self.n)])

        bendtwist1 = self.B_vorn_org * self.k / self.e_vorn[:, None]**3
        bendtwist2 = np.cross(
            self.k, self.B_vorn_org/self.e_vorn[:, None]**3 * self.k) \
            * self.D_org[:, None]
        ...
        unsteady_dilatation = self.J_org * self.w / self.e[:, None]**2 \
            * (self.e - self.e_prev)[:, None] / dt_prev

        cv = -self.gamma * self.w * self.e[:, None] * self.l_org[:, None]

        rhs = self.delta_h_op(bendtwist1) + self.A_h_op(bendtwist2) \
            + shearstretch + lagrg_transport + unsteady_dilatation \
            + self.c_ext + cv
