...
from cantilever_distrubuted_load_postprecessing import plot_video_with_surface, Find_Tip_Position,adjust_square_cross_section


def cantilever_subjected_to_a_nonconservative_load(n_elem,base_length,base_radius,youngs_modulus,dimentionless_varible,Animation=False,PLOT_FIGURE_Equilibrium=False):
    ...
    adjust_section = adjust_square_cross_section(n_elem,
        direction,
        normal,
        base_length,
        base_radius,
    ...
    load = (youngs_modulus * I * dimentionless_varible) / (density * base_area * (base_length ** 3))
    ...
    class AxialStretchingCallBack(ea.CallBackBaseClass):
        ...
        def make_callback(self, system, time, current_step: int):
            if current_step % self.every == 0:
                ...
                self.callback_params["velocity_magnitude"].append((stretchable_rod.velocity_collection[-1][0] ** 2 +
                                                                   stretchable_rod.velocity_collection[-1][1] ** 2 +
                                                                   stretchable_rod.velocity_collection[-1][
                                                                       2] ** 2) ** 0.5)
    ...
    if PLOT_FIGURE_Equilibrium:
        ...
        plt.plot(recorded_history["time"], recorded_history["velocity_magnitude"], lw=1.0, label="velocity_magnitude")
        plt.xlabel("Time (s)")
...
cantilever_subjected_to_a_nonconservative_load(n_elem,base_length,base_radius,youngs_modulus,-15,True,False)
...
for i in load_on_rod:
    x_tip_experiment.append(cantilever_subjected_to_a_nonconservative_load(n_elem,base_length,base_radius,youngs_modulus,i,False,False)[0])
    y_tip_experiment.append(-cantilever_subjected_to_a_nonconservative_load(n_elem,base_length,base_radius,youngs_modulus,i,False,False)[1])

plt.plot(load_on_rod, x_tip_paper, color='black', marker='*',linestyle='--', label='Theoretical_x')
plt.plot(load_on_rod, y_tip_paper, color='black', marker='*',linestyle=':', label='Theoretical_y')
plt.scatter(load_on_rod, x_tip_experiment, color='blue', marker='s',linestyle='None',label='x_tip/L')
plt.scatter(load_on_rod, y_tip_experiment, color='red', marker='s',linestyle='None',label='y_tip/L')
