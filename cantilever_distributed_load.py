...
def Conservative_Force_Simulator(load,Animation = False):
    ...
    base_length = 0.5
    base_radius = 0.01/(np.pi**(1/2))   #The Cross-sectional area is 1e-4(we assume its equivalent to a square cross-sectional surface with same area)
        ...
        def make_callback(self, system, time, current_step: int):
            if current_step % self.every == 0:
                ...
                self.callback_params["velocity_magnitude"].append((stretchable_rod.velocity_collection[-1][0] ** 2 +
                                                                   ...
                                                                   stretchable_rod.velocity_collection[-1][2] ** 2) ** 0.5)
    ...
    relative_tip_position[0] = Find_Tip_Position(stretchable_rod, n_elem)[0] / base_length
    relative_tip_position[1] = -Find_Tip_Position(stretchable_rod, n_elem)[1] / base_length
