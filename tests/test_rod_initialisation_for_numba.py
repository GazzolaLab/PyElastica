...
try:
    ...
    def test_straight_rod():
        ...
        test_rod = factory_function.allocate(
            n,
            start,
            direction,
            normal,
        ...
        assert_allclose(
            test_rod.position_collection[..., 0], start, atol=Tolerance.atol()
        )
        ...
        assert_allclose(
            test_rod.omega_collection, np.zeros((3, n)), atol=Tolerance.atol()
        )
        assert_allclose(test_rod.rest_sigma, np.zeros((3, n)), atol=Tolerance.atol())
        assert_allclose(
            test_rod.rest_kappa, np.zeros((3, n - 1)), atol=Tolerance.atol()
        )
        ...
        for i in range(n):
            ...
            assert_allclose(
                test_rod.rest_lengths, base_length / n, atol=Tolerance.atol()
            )
            assert_allclose(
