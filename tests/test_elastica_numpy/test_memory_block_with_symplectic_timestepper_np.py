...
from elastica._elastica_numpy._rotations import _get_rotation_matrix
...
def test_block_structure_dynamic_state_kinematic_rates(n_rods):
    ...
    assert_allclose(
        prefac * block_structure.velocity_collection,
        block_structure.kinematic_rates(0, prefac)[0],
        atol=Tolerance.atol(),
    )
    assert_allclose(
        prefac * block_structure.omega_collection,
        block_structure.kinematic_rates(0, prefac)[1],
        atol=Tolerance.atol(),
    )
...
def test_block_structure_dynamic_update(n_rods):
    ...
    block_structure.dynamic_states += block_structure.dynamic_rates(0, prefac)
