...
@pytest.mark.parametrize("n_elems", [5, 10, 50])
def test_constant_nu(n_elems):
    """
    This function tests, for fix dissipation
    constant input, validty of dissipation constant array
...
@pytest.mark.xfail(raises=AssertionError)
@pytest.mark.parametrize("n_elems", [5, 10, 50])
def test_nu_invalid_shape(n_elems):
    """
    This test is checking if user gives nu array in incorrect
    format and program throws an assertion error.
    Parameters
    ----------
    n_elems
...
def test_straight_rod(n_elems):
    ...
    assert_allclose(mockrod.nu, nu, atol=Tolerance.atol())
    assert_allclose(
