...
def test_straight_rod():
    ...
    assert_allclose(test_rod.position[..., 0], start,
                    atol=Tolerance.atol())
    rod_length = np.linalg.norm(test_rod.position[..., -1]
                                - test_rod.position[..., 0])
    # checking velocities, omegas and rest strains
    # density and mass
    assert_allclose(rod_length, base_length,
                    atol=Tolerance.atol())
    assert_allclose(test_rod.velocity, np.zeros((3, n + 1)),
                    atol=Tolerance.atol())
    assert_allclose(test_rod.omega, np.zeros((3, n)),
                    atol=Tolerance.atol())
    assert_allclose(test_rod.rest_sigma, np.zeros((3, n)),
                    atol=Tolerance.atol())
    ...
    for i in range(n):
        assert_allclose(test_rod.mass[i], mass,
                        atol=Tolerance.atol())
        assert_allclose(test_rod.directors[0, :, i],
                        normal, atol=Tolerance.atol())
        assert_allclose(test_rod.directors[1, :, i],
                        direction, atol=Tolerance.atol())
        assert_allclose(test_rod.directors[2, :, i],
        ...
        assert_allclose(test_rod.shear_matrix[..., i],
                        shear_matrix,
                        atol=Tolerance.atol())
        assert_allclose(test_rod.mass_second_moment_of_inertia[..., i],
