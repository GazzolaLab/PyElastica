...
def test_freejoint():
     ...
     # create rod classes
     rod1 = create_straight_rod(n, origin1, direction, normal, base_length, base_radius, density,
                               mass_second_moment_of_inertia, shear_matrix, bend_matrix)
     rod2 = create_straight_rod(n, origin2, direction, normal, base_length, base_radius, density,
                                mass_second_moment_of_inertia, shear_matrix, bend_matrix)
...
def test_hingejoint():
     ...
     direction2 = np.array([1.0/np.sqrt(2),1.0/np.sqrt(2),0.])
     ...
     # create rod classes
     rod1 = create_straight_rod(n, origin1, direction, normal1, base_length, base_radius, density,
                               mass_second_moment_of_inertia, shear_matrix, bend_matrix)
     rod2 = create_straight_rod(n, origin2, direction2, normal2, base_length, base_radius, density,
                                mass_second_moment_of_inertia, shear_matrix, bend_matrix)
     ...
     assert_allclose(hgjt.rod_one.external_forces[..., rod1_index], contactforce, atol=Tolerance.atol())
     assert_allclose(hgjt.rod_two.external_forces[..., rod2_index], -1 * contactforce, atol=Tolerance.atol())
...
def test_fixedjoint():
     ...
     fxjt = HingeJoint(k, nu, rod1, rod2, rod1_index, rod2_index, kt, normal1)

     fxjt.apply_force()
     fxjt.apply_torque()

     assert_allclose(fxjt.rod_one.external_forces[..., rod1_index], contactforce, atol=Tolerance.atol())
     assert_allclose(fxjt.rod_two.external_forces[..., rod2_index], -1 * contactforce, atol=Tolerance.atol())
