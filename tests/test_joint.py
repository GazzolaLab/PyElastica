...
def test_freejoint():
     ...
     class rod:
         def __init__(self, n,r,v):
             ...
             self.external_torques =np.tile(np.array([0.0,0.0,0.0]), (n,1))
...
class rod:
    def __init__(self, n, r, v):
        self.position = r
        self.velocity = np.tile(v, (n + 1, 1))
        self.directors = np.tile(np.array(Q),(n,1))
        self.external_forces = np.tile(np.array([0.0, 0.0, 0.0]), (n + 1, 1))
        self.external_torques = np.tile(np.array([0.0, 0.0, 0.0]), (n, 1))
...
Q = np.transpose(np.array([d1,d2,d3]))
...
x = np.linspace(0., L, n + 1)
r1 = np.transpose(np.array([[i, 0., 0.] for i in x]) + origin1)
r2 = np.transpose(np.array([[i, i, 0.] for i in x]) + origin2)
...
hgjt = HingeJoint(k, nu, rod1, rod2, rod1_index, rod2_index, kt, normal_direction)
